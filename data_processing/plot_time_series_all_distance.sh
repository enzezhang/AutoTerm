...
area_change_Landsat5=$path/${GID}_Landsat5.txt
area_change_Landsat7=$path/${GID}_Landsat7.txt
...
read min max<<<$(python $script --input temp.txt)
 echo $min
 echo $max
min_new=$(echo $min|awk '{print $1*0.7}')
...
# echo $max_GID
#R=1984/2021/$min_new/$max_new
R=1984/2021/0/6000
J=X12i/4i
D=jTL+w1c+o0.2c/0.2c
adjustment=11000
gmt psbasemap -J$J -R$R -Ba5/a500WSne -K -P -X2i -Y5.5i> $PS
...
cat $area_change_Landsat5 | awk '{print $2,$1-adjustment,error_Landsat_plot}' adjustment="$adjustment"  error_Landsat_plot="$error_Landsat" |gmt psxy -J$J -R$R -Sc0.1i -W1p,black -Ey+p1p -K -O >> $PS

cat $area_change_Landsat7 | awk '{print $2,$1-adjustment,error_Landsat_plot}' adjustment="$adjustment" error_Landsat_plot="$error_Landsat" |gmt psxy -J$J -R$R -Sc0.1i -W1p,blue -Ey+p1p -K -O >> $PS
cat $area_change_Landsat8 | awk '{print $2,$1-adjustment,error_Landsat_plot}' adjustment="$adjustment" error_Landsat_plot="$error_Landsat" |gmt psxy -J$J -R$R -Sc0.1i -W1p,red -Ey+p1p -K -O >> $PS

cat $area_change_sentinel1a | awk '{print $2,$1-adjustment,error_S1A_plot}' adjustment="$adjustment" error_S1A_plot="$error_Landsat" |gmt psxy -J$J -R$R -Sc0.1i -W1p,green -Ey+p1p  -K -O >> $PS
cat $area_change_sentinel1d| awk '{print $2,$1-adjustment,error_S1D_plot}' adjustment="$adjustment" error_S1D_plot="$error_Landsat"|gmt psxy -J$J -R$R -Sc0.1i  -W1p,green -Ey+p1p -K -O >> $PS
cat $area_change_sentinel2 | awk '{print $2,$1-adjustment,error_Landsat_plot}' adjustment="$adjustment" error_Landsat_plot="$error_Landsat"|gmt psxy -J$J -R$R -Sc0.1i -Ey+p1p -W1p,purple  -K -O >> $PS



echo 1990 $max_GID $GID | gmt pstext -R$R -J$J -F+f22p,1,black -K -O >>$PS
...
echo 0.2 0.4 "Distance (meter)" | gmt pstext -J$Jl -R$Rl -F+f22p,0,black+a90+jTL -O -X-1.8i -Y-1i  >>$PS
