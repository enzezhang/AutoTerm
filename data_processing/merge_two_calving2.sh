...
path=$1
out_path=$2
script=/home/staff/enze/Front_DL3/data_processing/invert_data2.py
echo $out_path
if [ -d $out_path ];then
        echo "dir exist"
else
...
do
        if [ -f $out_path/${file[i]} ];then
                ...
                temp=(`echo ${file[i]}| cut -d '.' -f 1`)
                        python $script --input ${file[i]} --output ${file[i]}.invert
                if [ $i = 0 ];then
		...
		#gmt gmtset FORMAT_GEO_OUT=ddd.xxx
                #gmt gmt2kml -Fp $out_path/${file[i]} > $out_path/$temp.kml
