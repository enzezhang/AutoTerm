...
y_interval=$(echo "(($max_new)-($min_new))"|bc | awk '{printf("%.2f\n"),$1/5}')
temp=$(echo "(($max_new)-($min_new))"|bc | awk '{printf("%.2f\n"),$1*0.7}')

max_GID=$(echo "(($min_new)+($temp))"|bc | awk '{printf("%.2f\n"),$1}')
# echo $max_GID
R=1984/2013/$min_new/$max_new
J=X12i/4i
D=jTL+w1c+o0.2c/0.2c

gmt psbasemap -J$J -R$R -Ba5/a${y_interval}WSne -K -P -X2i -Y5.5i> $PS
...
cat $area_change_Landsat5 | awk '{print $3,$1,error_Landsat_plot}' error_Landsat_plot="$error_Landsat" |gmt psxy -J$J -R$R -Sc0.1i -W1p,red -Ey+p1p -K -O >> $PS

cat $area_change_Landsat7 | awk '{print $3,$1,error_Landsat_plot}' error_Landsat_plot="$error_Landsat" |gmt psxy -J$J -R$R -Sc0.1i -W1p,blue -Ey+p1p -K -O >> $PS
